...
Our first functions allows us to easily view what measures we can query, along with a brief definition. If we run `get_measures`, we can view the measures in a data frame in the R Studio viewer. If we instead print `measures`, the pre-loaded data set, we see a preview.

```{r}
head(measures) %>% tibble()
```
